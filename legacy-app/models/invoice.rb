...
class Invoice < ApplicationRecord
  ...
  enum status: %i[pending_invoice open submitted paid partially_paid cancelled]
  enum invoice_type: %i[timesheet_invoice client_expense_invoice]
  ...
  scope :open_invoices, -> { where(status: %i[pending_invoice open]) }
  scope :cleared_invoices, -> { where(status: [:paid]) }
  scope :submitted_invoices, -> { where(status: :open) }
  scope :all_invoices, -> { where(status: %i[open submitted paid partially_paid cancelled]) }
  ...
  def set_number
    i = contract.invoices.order('created_at DESC').first
    self.number = if i.present?
                    'IN_' + contract.only_number.to_s + '_' + (i.only_number.to_i + 1).to_s.rjust(3, '0')
                  else
                    'IN_' + contract.only_number.to_s + '_001'
                  end
  end
  ...
  def set_seq_accept_in
    ...
    tx = ledger.transactions.transact do |builder|
      builder.issue(
        flavor_id: 'tym',
        amount: (total_amount.to_i * 100).to_i,
        destination_account_id: "#{contract.buy_contract.candidate.full_name.parameterize + contract.buy_contract.candidate.id.to_s}_exp",
        action_tags: {
          'Fixed' => 'false',
          'Status' => 'open',
          'Account' => '',
          'CycleId' => ig_cycle_id.to_s,
          'ObjType' => 'IG',
          'ContractId' => contract_id.to_s,
          'PostingDate' => Time.now.strftime('%m/%d/%Y'),
          'CycleFrom' => start_date.strftime('%m/%d/%Y'),
          'CycleTo' => end_date.strftime('%m/%d/%Y'),
  ...
  def set_seq_paid_in
    ...
    tx = ledger.transactions.transact do |builder|
      builder.transfer(
        flavor_id: 'tym',
        amount: (total_amount.to_i * 100).to_i,
        destination_account_id: "#{contract.sell_contract.company.slug.to_s + contract.sell_contract.company.id.to_s}_q",
        source_account_id: "#{contract.buy_contract.candidate.full_name.parameterize + contract.buy_contract.candidate.id.to_s}_exp",
        action_tags: {
          'Fixed' => 'false',
          'Status' => 'Clear',
          'Account' => '',
  ...
  def set_consultant_and_total_amount
    ...
    contract.timesheets.approved.not_invoiced.each { |t| total_time_in_sec += t.approved_total_time }
  ...
  def start_date_cannot_be_less_than_end_date
    errors.add(:start_date, ' cannot be less than end date.') if end_date < start_date
  end

  def contract_validation
    errors.add(:base, "Contract is #{contract.status.humanize}")
  end
  ...
  def set_commissions
    ...
    if contract.is_commission
      if contract.fixed? && contract.invoices.empty?
        commission = contract.commission_amount
      elsif contract.percentage? && contract.invoices.sum(:commission_amount) < contract.max_commission
        commission = total_amount * 0.01 * contract.commission_amount
        commission = commission + contract.invoices.sum(:commission_amount) > contract.max_commission ? (contract.max_commission - contract.invoices.sum(:commission_amount)) : commission
      end
    end
    self.commission_amount = commission
    self.billing_amount = total_amount - commission_amount - consultant_amount
  end
  ...
  def set_next_invoice_date
    ...
    temp_date = contract.next_invoice_date + contract.sell_contract.invoice_terms_period.to_i.days # TIMESHEET_FREQUENCY[self.contract.time_sheet_frequency].days
    contract.next_invoice_date = temp_date > contract.end_date ? contract.end_date : temp_date
    contract.save
  end
  ...
  def update_timesheet_status_to_invoiced
    ...
    timesheets.each do |t|
      t.update_attributes!(invoice_id: id, status: 'invoiced')
    end
  end

  def set_start_date_and_end_date
    self.start_date = contract.invoices.empty? ? contract.start_date : (contract.invoices.last.end_date + 1)
    self.end_date = contract.next_invoice_date
  end

  def set_rate
    self.rate = contract.rate
  end

  def create_invoice_for_parent
    invoice = contract.parent_contract.invoices.new
    invoice.start_date = start_date
    invoice.end_date = end_date
    invoice.total_approve_time = total_approve_time
    invoice.total_amount = invoice.contract.rate * (total_approve_time / 3600.0)
    invoice.consultant_amount = total_amount
    invoice.billing_amount = invoice.total_amount - invoice.consultant_amount
    invoice.parent_id = id
    invoice.save
  end

  def notify_contract_responder
    contract.respond_by.notifications.create(message: "Your <a href='http://#{contract.respond_by.company.etyme_url + contract_invoice_path(contract, self)}'>Invoice</a> for <a href='http://#{contract.respond_by.company.etyme_url + contract_path(contract)}'>contract</a>", title: "Invoice# #{id}")
  end

  def notify_contract_creator
    contract.created_by.notifications.create(message: "Your <a href='http://#{contract.created_by.company.etyme_url + contract_invoice_path(contract, self)}'>Invoice</a> for <a href='http://#{contract.created_by.company.etyme_url + contract_path(contract)}'>contract</a>", title: "Invoice# #{id}")
  end
  ...
  def self.set_con_cycle_invoice_date(sell_contract, con_cycle)
    ...
    # binding.pry
    con_cycle_invoice_start_date = case @invoice_type
                                   when 'daily'
                                     con_cycle&.start_date
                                   when 'weekly'
                                     date_of_next(@invoice_day_of_week, con_cycle)
                                   # binding.pry
                                   when 'monthly'
                                     # binding.pry
                                     if @invoice_end_of_month
                                       con_cycle.start_date.end_of_month
                                     else
  ...
  def self.montly_approval_date(_con_cycle)
    # binding.pry
