...
class Company::ClientExpensesController < Company::BaseController
  ...
  before_action :set_client_expense, only: %i[edit update submit reject approve]
  ...
  def index
    ...
    @client_expenses = @ts_for == 'sent_client_expenses' ?
                           ...
                           # ClientExpense.where.not(status: %i[pending_expense not_submitted]).joins(contract_cycle: [contract: :sell_contract])
                           ...
                           #              .between_date(@start_date, @end_date)
                           #              .paginate(page: params[:page], per_page: 10).order(start_date: :asc)
                           Expense.joins(:contract).where(contract_id: current_company.contracts).paginate(page: params[:page], per_page: 10)
  end
  ...
  def client_expenses_params
    params.require(:client_expense).permit(expense_items_attributes: %i[id quantity unit_price expense_type description])
  end
