class Company::ContractsController < Company::BaseController
  ...
  def index
    ...
    if params[:page]
      @contract_activity = PublicActivity::Activity.where(trackable: current_company.contracts).order('created_at DESC').paginate(page: params[:page], per_page: 15) if params[:tab].to_i == 1
      @sent_contracts = @sent_search.result.paginate(page: params[:page], per_page: 15) || [] if params[:tab].to_i == 2
      @buy_contracts = Contract.joins(:buy_contract).where(buy_contracts: {company_id: current_company.id}).order('created_at DESC').paginate(page: params[:page], per_page: 15) if params[:tab].to_i == 3
      @sell_contracts = Contract.joins(:sell_contract).where(sell_contracts: {company_id: current_company.id}).order('created_at DESC').paginate(page: params[:page], per_page: 15) if params[:tab].to_i == 4
    end
  end
