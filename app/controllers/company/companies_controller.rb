...
class Company::CompaniesController < Company::BaseController
  ...
  def update_banner_title
    @company = Company.find_by_id(params[:id])
    @company&.update_attributes(banner_title: params['custom_domain'])
  end
  ...
  def company_params
    params.require(:company).permit(:name, :company_type, :domain, :skill_list, :website, :logo, :description, :phone, :email, :linkedin_url, :facebook_url, :twitter_url, :google_url, :is_activated, :status, :time_zone, :tag_line, :custom_domain, :banner_title, :banner_text, :banner_btn_label, :banner_btn_url, group_ids: [], owner_attributes: %i[id type first_name last_name email password password_confirmation], locations_attributes: [:id, :name, :status, address_attributes: %i[id address_1 country city state zip_code]], invited_by_attributes: %i[invited_by_company_id user_id])
  end

  def create_params
    params.require(:company).permit([:name, :email, :domain, :company_type, :currency_id, :phone, :fax_number, :send_email, :slug, :website, :custom_domain, :banner_title, :banner_text, :banner_btn_label, :banner_btn_url, group_ids: [],
