...
class Company::CompaniesController < Company::BaseController
  ...
  def index
    ...
    respond_to do |format|
      format.html {}
      format.json { render json: CompanyDatatable.new(params, user:current_user, view_context: view_context) }
    end
  end
  ...
  def create_params
    params.require(:company).permit([:name, :email, :domain, :company_type, :currency_id, :phone, :fax_number, :send_email, :slug, :website, :custom_domain, :banner_title, :banner_text, :banner_btn_label, :banner_btn_url, :banner_two_btn_label, :banner_two_btn_url, :banner_color, :show_slider, :apply_gradient, :show_name_logo, group_ids: [],
