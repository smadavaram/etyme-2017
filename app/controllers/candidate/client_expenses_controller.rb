class Candidate::ClientExpensesController < Candidate::BaseController
  ...
  def index
    ...
    @client_expenses = current_candidate.client_expenses.open_expenses.where("(start_date >= ? AND start_date <= ?) OR (end_date >= ? AND end_date <= ?) ",(@dates.beginning_of_week-1.day), (@dates.end_of_week - 1.day), (@dates.beginning_of_week-1.day), (@dates.end_of_week - 1.day))
  ...
  def get_client_expenses
    ...
    @client_expenses = current_candidate.client_expenses.open_expenses.where("(start_date >= ? AND start_date <= ?) OR (end_date >= ? AND end_date <= ?) ",@start_date, @end_date, @start_date, @end_date)
  end
  ...
  def update
    @client_expense = current_candidate.client_expenses.open_expenses.find(params[:id])
  ...
  def submitted_client_expenses
    @client_expenses = current_candidate.client_expenses.submitted_client_expenses
  end
