class Static::JobApplicationsController < ApplicationController
  ...
  def create
    ...
    if params[:candidate_id].present? || current_candidate
      ...
      @job_application.applicant_resume = @candidate.candidates_resumes.primary_resume&.resume if params[:primary_resume] and @candidate.candidates_resumes.primary_resume.present?
      save_job_application(@job_application)
    else
      candidate_with_recruiter
      if params["candidate_without_registration"].present?
        ...
        if params["job_application"]["candidate_with_recruiter"]["email"]
          ...
          if !companies.blank?
            ...
            @job_application = candidate.job_applications.new(job_application_params.merge(job_id: params[:job_id],application_type: :with_recurator,company_id: @job.company_id,recruiter_company_id: @company.id))
            save_job_application(@job_application)
          else
            @company = Company.new()
