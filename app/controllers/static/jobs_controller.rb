...
class Static::JobsController < ApplicationController
  ...
  def people
    ...
    if request.subdomain == "app"
      if candidate_ids.present?
        @candidates = CandidatesCompany.hot_candidate.where(candidate_id: candidate_ids).paginate(page: params[:page], per_page: 20)
      else
        @candidates = CandidatesCompany.hot_candidate.paginate(page: params[:page], per_page: 20)
      end
    elsif @current_company.present?
      if candidate_ids.present?
        @candidates = CandidatesCompany.hot_candidate.where(company_id: @current_company.id, candidate_id: candidate_ids).paginate(page: params[:page], per_page: 20)
      else
        @candidates = CandidatesCompany.hot_candidate.where(company_id: @current_company.id).paginate(page: params[:page], per_page: 20)
      end
    end
  ...
  def filter_jobs
    ...
    if request.subdomain == "app"
      if params[:selected_categories].present?
        @job_all = Job.active.is_public.where(job_category: params[:selected_categories]).order(created_at: :desc).paginate(page: params[:page], per_page: 20) || []
      else
        @job_all = Job.active.is_public.order(created_at: :desc).paginate(page: params[:page], per_page: 20) || []
      end
    else
      if params[:selected_categories].present?
        @job_all = @current_company.jobs.active.is_public.where(job_category: params[:selected_categories]).order(created_at: :desc).paginate(page: params[:page], per_page: 20) || []
      else
        @job_all = @current_company.jobs.active.is_public.order(created_at: :desc).paginate(page: params[:page], per_page: 20) || []
      end
    end
  end
  ...
  def set_jobs
    ...
    if request.subdomain == "app"
      ...
      jobs = @search.result(distinct: true).order(created_at: :desc).paginate(page: params[:page], per_page: 20)
