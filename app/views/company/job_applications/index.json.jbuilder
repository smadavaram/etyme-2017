...
json.data @job_applications do |application|
  ...
  json.action               application.pending? ? "<button class='btn btn-xs'>Open case</button> #{link_to 'Reject' , reject_job_application_job_job_invitation_job_application_path(application.job  , application.job_invitation , application) , method: :post , remote: true , class:'btn btn-xs btn-danger pull-right' , style: 'margin-left: 5px'} #{link_to 'Short List' , short_list_job_application_job_job_invitation_job_application_path(application.job  , application.job_invitation , application) , method: :post , remote: true , class:'btn btn-xs btn-primary pull-right' , style: "margin-left: 5px; "} #{link_to 'Accept' , accept_job_application_job_job_invitation_job_application_path(application.job  , application.job_invitation , application) , method: :post , remote: true , class:'btn btn-xs btn-success pull-right'}" : "<button class='btn btn-xs'>#{application.status}</button>"
end
