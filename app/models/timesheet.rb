class Timesheet < ApplicationRecord
  ...
  scope :all_timesheets, -> { where(status: [:submitted, :approved, :rejected]) }
  ...
  scope :between_date, -> (start_date, end_date) { start_date.present? and end_date.present? ? where('timesheets.start_date BETWEEN ? AND ?', start_date, end_date).or(where('timesheets.end_date BETWEEN ? AND ?', start_date, end_date)) : nil }
