class Candidate < ApplicationRecord
  ...
  scope :search_by, ->term,search_scop{Candidate.joins(:skills).where("lower(tags.name) like '%#{term.downcase}%'")+Candidate.joins(:skills).where("tags.name like '%#{term}%'")+ Candidate.where('lower(first_name) like :term or lower(last_name) like :term or lower(phone) like :term or lower(email) like :term ', {term: "%#{term.downcase}%"})}
