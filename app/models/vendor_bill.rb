...
class VendorBill < ApplicationRecord
  def self.set_con_cycle_vp_pro_date(buy_contract, con_cycle)
    ...
    @vp_day_of_week = if buy_contract&.ir_day_of_week.present?
                        Date.parse(buy_contract&.ir_day_of_week&.titleize).try(:strftime, '%A')
                      else
                        'mon'
                      end
    ...
    con_cycle_vp_start_date = case @vp_type
                              when 'daily'
                                con_cycle.start_date
                              when 'weekly'
                                date_of_next(@vp_day_of_week, con_cycle)
                              when 'monthly'
                                if @end_of_month
