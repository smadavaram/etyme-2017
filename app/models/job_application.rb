...
class JobApplication < ApplicationRecord
  ...
  def update_job_invitation_status
    ...
    job.created_by.notifications.create(notification_type: :application, createable: applicationable, message: applicationable.full_name + " has #{job_invitation.status.humanize} <a href='http://#{job.created_by.company.etyme_url + static_job_url(self.job).to_s}}'> on your Job </a>#{job.title}", title: 'Job Invitation')
  end

  # Call after update
  def notify_recipient_on_status_change
    applicationable.notifications.create(notification_type: :application, createable: job.company.owner, message: "#{job&.company&.name.to_s} has #{status&.humanize} <a href='http://#{applicationable&.etyme_url.to_s} #{static_job_url(self.job).to_s}}'> Job Application </a> #{job&.title}".to_s, title: 'Job Application')
  end
  ...
  def notify_job_owner_or_admins
    if applicationable.class.name == 'Candidate'
      job.created_by.notifications.create(notification_type: :application, createable: applicationable, message: applicationable.full_name + " has <a href='http://#{job.created_by.company.etyme_url + static_job_url(self.job).to_s}}'>apply</a> to your Job Application - #{job.title}", title: 'Job Application')
    else
      job.company.all_admins_has_permission?('manage_job_applications').each do |admin|
        if applicationable_id.blank?
          admin.notifications.create(notification_type: :application, createable: applicationable, message: " has <a href='http://#{job.created_by.company.etyme_url + static_job_url(self.job).to_s}}'>apply</a> your Job Application - #{job.title}", title: 'Job Application')
        else
          admin.notifications.create(notification_type: :application, createable: applicationable, message: applicationable.company.name + " has <a href='http://#{job.created_by.company.etyme_url + static_job_url(self.job).to_s}}'>apply</a> your Job Application - #{job.title}", title: 'Job Application')
        end
      end
    end
  end
