...
class JobInvitation < ApplicationRecord
  ...
  def notify_recipient
    if recipient_type == 'Candidate'
      job ?
          recipient.notifications.create(message: company.name + " has invited you for <a href='http://#{recipient.etyme_url + job_invitation_path(self)}'>#{job&.title}</a> <br/> <p> #{message} </p>", title: 'Job Invitation', createable: created_by)
          :
