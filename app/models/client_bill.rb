...
class ClientBill < ApplicationRecord
  def self.set_con_cycle_cp_pro_date(buy_contract, con_cycle)
    ...
    @cp_day_of_week = if buy_contract&.cp_day_of_week.present?
                        Date.parse(buy_contract&.cp_day_of_week&.titleize).try(:strftime, '%A')
                      else
                        'mon'
                      end
    ...
    con_cycle_cp_start_date = case @cp_type
                              when 'daily'
                                con_cycle.start_date
                              when 'weekly'
                                date_of_next(@cp_day_of_week, con_cycle)
                              when 'monthly'
                                if @end_of_month
