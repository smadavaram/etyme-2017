class Job < ApplicationRecord
  ...
  scope :search_by, ->term,search_scop { Job.joins(:tags).where("tags.name like '%#{term}%'")+(Job.where('lower(title) like :term or lower(description) like :term or lower(location) like :term or lower(job_category) like :term', {term: "#{term&.downcase}%"}))}
