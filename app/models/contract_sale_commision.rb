...
class ContractSaleCommision < ApplicationRecord
  ...
  def self.set_con_cycle_com_pro_date(buy_contract, con_cycle)
    ...
    @com_pro_day_of_week = if buy_contract&.com_pro_day_of_week.present?
                             Date.parse(buy_contract&.com_pro_day_of_week&.titleize).try(:strftime, '%A')
                           else
                             'mon'
                           end
    ...
    con_cycle_com_pro_start_date = case @com_pro_type
                                   when 'daily'
                                     con_cycle.start_date
                                   when 'weekly'
                                     date_of_next(@com_pro_day_of_week, con_cycle)
                                   when 'monthly'
                                     if @end_of_month
