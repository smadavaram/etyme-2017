...
module ApplicationHelper
  ...
  def contact_widget(email, phone, user_id = nil, options = {})
    chat_remote_link(options) +
        ...
        link_to(content_tag(:i, nil, class: 'os-icon os-icon-phone ').html_safe, 'skype:live:amirad2020?call', title: phone, class: 'data-table-icons') +
        ...
        users.signers&.take(3).each do |signer|
          user_photo=user_photo+"<div class='table_avatar'>"
            if signer.photo.blank?
              user_photo=user_photo+ entity_image(signer.first_name,signer.last_name,'avatar_circle')
            else
