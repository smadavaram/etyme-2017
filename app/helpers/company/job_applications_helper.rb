module Company::JobApplicationsHelper
  def job_application_top_links(app)
    ...
    btn += link_to 'Start Contract' , accept_job_application_path(app) , method: :post , remote: true ,   class: "contract btn btn-md btn-default mt-12 margin-right-5 #{ !(app.hired? && app.contract.blank?) ? 'disabled' : "" }"
    btn.to_s
  end
end
