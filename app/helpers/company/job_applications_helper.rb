module Company::JobApplicationsHelper
  def job_application_top_links(app)
    ...
    if  app.job.company == current_company && has_permission?('manage_job_applications')
      ...
      btn += link_to 'Reject' , reject_job_application_path(app), method: :post  , class: "btn btn-md  btn-primary mt-12 margin-right-5 #{app.hired? || app.contract.present? ? 'disabled' : "" }"
