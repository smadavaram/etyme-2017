...
checkForConScrollEvent = function (el) {
    ...
    more_users_url = $('#all-infinite-scrolling-con .pagination a[rel=next]').attr('href');
    var requestable_height = ($('.company-conversation-scorll')[0].scrollHeight - $('.company-conversation-scorll').height()) - el.scrollTop();
    if (more_users_url && requestable_height === 100) {
        $('#loading').removeClass('hidden');
