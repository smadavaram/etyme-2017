class CompanyDatatable < ApplicationDatatable
  ...
  def reminder_note record
    content_tag(:span, record.reminders.where(user_id: current_user)&.last&.title, class: 'label-info badge mr-1').html_safe
  end

  def contact_icon record
    mail_to(record.owner&.email, content_tag(:i, nil, class: 'fa fa-envelope').html_safe, title: record.email, class: 'data-table-icons') +
        ...
        link_to(content_tag(:i, nil, class: 'fa fa-comment-o').html_safe, '#', title: 'Add meeting', class: 'data-table-icons')
