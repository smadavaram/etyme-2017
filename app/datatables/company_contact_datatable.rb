class CompanyContactDatatable < ApplicationDatatable
  ...
  def company_user_profile user
    (link_to user_image(user, style: 'width: 35px; height: 35px;', class: 'data-table-image mr-1',title: "#{user.full_name}" ),company_user_profile_path(user) )+
     link_to(do_ellipsis(user.first_name), company_user_profile_path(user), class: 'data-table-font pl-2')

  end

  def company_profile company
    image_tag(company.logo, class: 'data-table-image mr-1',title: "#{company.name}").html_safe +
