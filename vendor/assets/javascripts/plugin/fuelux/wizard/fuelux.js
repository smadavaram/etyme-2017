...
+function ($) { "use strict";
    ...
    Pillbox.prototype = {
        ...
        items: function() {return this.$element.find('li').map(function() {
            var $this = $(this);
        ...
        itemclicked: function (e) {
            var custom_tag = $(e.currentTarget).parents('.form-group').find('.custom-tag');
            custom_tag.val(custom_tag.val().replace(","+$(e.currentTarget).html(), ''))
            custom_tag.val(custom_tag.val().replace($(e.currentTarget).html()+",", ''))
            custom_tag.val(custom_tag.val().replace($(e.currentTarget).html(), ''))
...
$(function() {
    var addPill = function($input){
        ...
        $item = $('<li class="bg-success">'+$text+'</li> ');
        $item.insertBefore($input);

        custom_tag = $input.parents('.form-group').find('.custom-tag');
        if(custom_tag.val() == ""){
            custom_tag.val($text);
        }
        else{
            custom_tag.val(custom_tag.val() + "," + $text);
        }
        $input.val("");
...
function validateForm(){
    is_valid = true;
    if($('.form-with-jquery-validate').length > 0 ){
        if(!$('.form-with-jquery-validate').valid()){
            $.amaran({
                content: {
                    bgcolor: '#D04B2B',
                    color: '#fff',
        ...
        if($('.step-content').find('.active').find('.btn-custom-file').length > 0){
            $('.step-content').find('.active').find('.btn-custom-file:input[type="filepicker"]').each(function( index ) {
                if($(this).val() == '') {
                    $.amaran({
                        content: {
                            bgcolor: '#D04B2B',
        ...
        if($('.step-content').find('.active').find('.signature').length > 0){
            if ($('.step-content').find('.active').find('.signature').val() == '') {
                $.amaran({
                    content: {
                        bgcolor: '#D04B2B',
                        color: '#fff',
                        message: "Please sign below to continue."
                    },
                    theme: 'colorful',
                    position: 'top right',
                });
        ...
        if($('.step-content').find('.active').find('.working-days-check-box').length > 0){
            if ($('.step-content').find('.active').find('.working-days-check-box:checked').length <= 0) {
                $.amaran({
                    content: {
                        bgcolor: '#D04B2B',
                        color: '#fff',
                        message: "Please select working days before continuing."
                    },
                    theme: 'colorful',
                    position: 'top right',
                });
